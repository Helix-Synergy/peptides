...
const Footer = () => {
  ...
  useEffect(() => {
    const fetchVisitorCount = async () => {
      try {
        const hasVisited = sessionStorage.getItem('visitedPharmameet');
        const queryParams = hasVisited ? '' : '?increment=true';
        
        if (!hasVisited) {
          sessionStorage.setItem('visitedPharmameet', 'true');
        }
        
        const API_URL = import.meta.env.VITE_API_BASE_URL;
        const response = await fetch(`${API_URL}/pharmameet-visitors${queryParams}`);
      ...
      <div className="container">
        <div className="footer-content">
          ...
          <div className="footer-col footer-col-logo">
            <div className="footer-logo-box">
              <img src={logo} alt="PharmaMeet 2026 Logo" className="footer-logo-img" />
            </div>
            <p className="footer-tagline">
              "Securing the Future with Pharmaceutical Sciences"
            </p>
        ...
        <div className="footer-bottom-bar">
          <div className="copyright-text">
            &copy; {new Date().getFullYear()} PharmaMeet. All Rights Reserved.
          </div>
